...
public class BookSuggestionConverter {
    ...
    public static BookSuggestionDTO convertToDTO(Suggestion suggestion, int upvoteCount, int publisherInterestCount, boolean userHasUpvoted, PublisherAction publisherAction, LocalDateTime publisherActionDate) {
    ...
    public static PublisherSuggestionView convertToPublisherView(Suggestion suggestion, int totalUpvotes, int totalPublisherInterests, PublisherSuggestionAction publisherAction) {
    ...
    public static List<BookSuggestionDTO> convertToDTOList(List<Suggestion> suggestions, Function<Suggestion, Integer> upvoteCountProvider, Function<Suggestion, Integer> interestCountProvider, Function<Suggestion, Boolean> userVoteProvider, Function<Suggestion, PublisherAction> actionProvider, Function<Suggestion, LocalDateTime> actionDateProvider) {
        return suggestions.stream().map(suggestion -> convertToDTO(suggestion, upvoteCountProvider.apply(suggestion), interestCountProvider.apply(suggestion), userVoteProvider.apply(suggestion), actionProvider.apply(suggestion), actionDateProvider.apply(suggestion))).collect(Collectors.toList());
    }
}